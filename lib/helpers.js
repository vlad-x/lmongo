...
exports.mergeSearchBody = function (searchOpts) {
    ...
    var body = {
        query: {
            multi_match: {
                // lowercase the query since we index with the `lowercase` filter
                query: searchOpts.query.toLowerCase(),
    ...
    if (searchOpts.where && Object.keys(searchOpts.where).length) {
        ...
        if (mustFilters.length) {
            ...
            body.filter.and[0].bool.must = mustFilters
        }

        if (shouldFilters.length) {
            body.filter.and[0].bool.should = shouldFilters
        }
    }
...
exports.doSearchAndNormalizeResults = function (searchUri, searchOpts, cb) {
    ...
    exports.backOffRequest(reqOpts, function (err, res, body) {
        ...
        if (body.hits.hits && body.hits.hits.length) {
            searchResults.hits = body.hits.hits.map(function (hit) {
                return hit._source
            })
        }
